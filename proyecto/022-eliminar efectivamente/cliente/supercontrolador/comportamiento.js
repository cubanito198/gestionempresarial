        ...
        .then(datos => {
            ...
            datos.forEach(function(tabla) { // Para cada una de las tablas que han venido de la base de datos
                ...
                elemento.onclick = function() { // Cuando hago click en los elementos de la tabla
                    console.log("Has hecho clic en la tabla:", this.textContent); // Añadir log para verificar
...
function cargaDatosTabla(tabla) {
    ...
    fetch("../../servidor/columnas_tabla.php?tabla=" + tabla) // LLamo a un microservicio que me da la lista de columnas y le paso la tabla como parámetro
        .then(response => {
            return response.json(); // Quiero que el servidor me devuelva un json
        })
        .then(datos => {
            ...
            datos.forEach(function(dato) { // Para cada uno de los datos
                ...
                if (dato['Key'] == "PRI") { // Si este campo es clave primaria
                    campoclave = dato['Field'] // en ese caso, recordamos cuál es el nombre del campo que hace de clave primaria
                }
            })
            ...
            fetch("../../servidor/datos_tabla.php?tabla=" + tabla) // LLamo a un microservicio que me da los datos de la tabla
                .then(response => {
                    return response.json(); // Quiero que el servidor me devuelva un json
                })
                .then(datos => {
                    ...
                    datos.forEach(function(registro) { // Como datos es un array, hago un forEach para repasarlo
                        ...
                        nuevacolumna.setAttribute("claveprimaria", clave_primaria) // Le pongo un atributo claveprimaria
                        ...
                        nuevacolumna.onclick = function() { // Cuando haga clic en la papelera
                            ...
                            fetch("../../servidor/eliminar_dato.php?tabla=" + tabla + "&id=" + identificador) // Elimino en el servidor
                            this.parentElement.remove() // Elimino el elemento visualmente
                        }
